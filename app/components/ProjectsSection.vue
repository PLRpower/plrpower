  ...
  <section id="projects" class="relative mt-48 md:mt-64" @mousemove="updateHoverPos">
    <div class="overflow-hidden relative">
      <div ref="projectsSectionRef" class="relative pt-24 pb-12">
        ...
        <div class="absolute -top-10 -left-4 md:left-10 font-display text-[20vw] md:text-[15vw] font-extrabold leading-none text-white/[0.02] select-none pointer-events-none z-0" aria-hidden="true">
          02
        </div>
        ...
        <div ref="horizontalRef" class="projects-horizontal-wrapper">
          <div ref="horizontalTrackRef" class="projects-track flex items-stretch gap-0">
            ...
            >
              ...
              <div v-if="!project.isViewAll" class="absolute inset-0 z-0 overflow-hidden bg-[#030303]">
                <AsciiImage 
                  v-if="project.image"
                  :src="project.image" 
                  :alt="project.title" 
                  ...
                  :group-hover="true"
                />
                <!-- Subdued bottom-left scrim for text contrast without hiding ASCII art -->
                <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_bottom_left,_rgba(3,3,3,0.5)_0%,_transparent_55%)] pointer-events-none"></div>
              </div>

              <!-- Project number for regular projects -->
              <span v-if="!project.isViewAll" class="absolute top-8 right-10 font-display text-[6rem] md:text-[8rem] font-extrabold leading-none text-white/[0.03] group-hover:text-white/[0.06] transition-colors duration-700 select-none z-10 pointer-events-none">
              ...
              <div v-if="!project.isViewAll" class="relative z-10 max-w-2xl">
                ...
                <p class="text-base text-secondary/95 leading-relaxed max-w-[500px] drop-shadow-sm">{{ project.description }}</p>
              </div>
